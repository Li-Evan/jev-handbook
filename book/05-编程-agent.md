...
![循环归大模型，Jev 只在四个接缝上答判断题](assets/figures/05-1.png)
...
![Alex Volkov 贴出的前后两张：压缩前上下文窗口用掉 904.7k，压缩后只剩 86.5k。截图能证明删掉了多少，证明不了删掉的东西后来用不用得上。图片来自 Alex Volkov 在 X 上的帖子](assets/shots/05-compaction-before-after.png)
...
![Jev 方案又快又省，留下的 token 却是线上方案的 2.1 倍](assets/figures/05-2.png)
...
![agent 手写的那段校验被 api-validation-uses-yup 这条规则拦下，概率 0.86，规则原文连同文件行号一起贴回工具结果里，agent 在同一轮改成了 Yup。图片来自 Abide 仓库的 docs/images/block.svg](assets/shots/05-abide-rule-block.png)
...
![Grok Build 的 25 个工具按 schema 大小排开，橙色是 Jev 为编程任务留下的 8 个；被删掉的长尾里，光 workflow 一个就占 1,397 个 token。图片来自 Daniel Farina 介绍 Nitro 的长文](assets/shots/05-nitro-tool-schemas.jpg)
...
![新增 1,161 行的那次：五道题只有测试覆盖以 0.960 过线，其余四道都没到 0.95，结论是 human_review_required。截自 mirrord 仓库第 4895 号 PR 下 Action 留的评论](assets/shots/05-jev-auto-approve.png)
