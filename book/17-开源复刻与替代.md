...
![接口抄得一样，概率从哪来、校没校准各不相同](assets/figures/17-1.png)
...
![同一台 3090、同一个冻结的 4B 模型、同样 21 个是非题：左边直接读概率，1.023 秒返回 21 组分布，一个 token 都不生成；右边让它写一个 yes/no 数组，要 5.332 秒、111 个 token。截自 SemIf 仓库里的对照演示页面](assets/shots/17-semif-replay.png)
...
![是非题整体差得不多，差距集中在最有把握的那批题](assets/figures/17-2.png)
...
![Laya README 里的自评图：灰色的 Jev 成绩是第三方公布的数字，图上自己注明 Jev 从没在这个项目里跑过；typed-decisions 那一栏是微调之后的 0.766 对 0.727。图片来自 Laya 仓库](assets/shots/17-laya-vs-jev.png)
...
![Kev 的 playground：一个请求里每道题各自给出完整的概率分布，最下面一排按钮可以把某道题的选项换个顺序重跑。截自 Kev 仓库 docs 目录里的截图](assets/shots/17-kev-playground.png)
